...
// === ONE-OFF: restore summary scores from PDF snapshot ===
// Accepts { rows: [{ company, contact, phone, language, nps, csat, support,
// renewal, callback }] }. Idempotent on phone match. For each row:
//   * If a response with matching phone exists: just add answers it lacks
//   * If not: insert new survey_responses row, then add answers
// To be removed after recovery completes.
app.post('/api/admin/_restore_pdf', requireAdmin, async (req, res) => {
  ...
  const out = { processed: 0, newResponses: 0, updatedIdentity: 0, answersInserted: 0, errors: [] };

  // Pre-fetch question_id by question_key from survey_questions (incl. retired)
  const qLookup = await pool.query(
    `SELECT id, key FROM survey_questions WHERE key IN ('nps','csat_product','support_score','renewal_intent','callback_request')`
  );
  const qIdByKey = Object.fromEntries(qLookup.rows.map((r) => [r.key, r.id]));

  for (const r of rows) {
    try {
      // Find existing response by phone match (the most stable identifier)
      const ex = await pool.query(
        `SELECT id, contact_name, position FROM survey_responses
      ...
      if (ex.rows.length > 0) {
        respId = ex.rows[0].id;
        if (!ex.rows[0].contact_name && r.contact) {
          await pool.query(`UPDATE survey_responses SET contact_name = $1 WHERE id = $2`,
            [r.contact, respId]);
          out.updatedIdentity += 1;
        }
      } else {
        const ins = await pool.query(
          `INSERT INTO survey_responses (company_name, contact_name, position, phone, language, created_at)
           VALUES ($1, $2, $3, $4, $5, $6) RETURNING id`,
          [r.company, r.contact || null, null, r.phone, r.language || 'ru', new Date('2026-04-27T09:00:00Z')]
        );
        respId = ins.rows[0].id;
        out.newResponses += 1;
      }

      const answerSpecs = [
        { key: 'nps',              value: r.nps },
        { key: 'csat_product',     value: r.csat },
        { key: 'support_score',    value: r.support },
        { key: 'renewal_intent',   value: r.renewal },
        { key: 'callback_request', value: r.callback },
      ];
      for (const spec of answerSpecs) {
        if (spec.value == null) continue;
        const has = await pool.query(
          `SELECT 1 FROM survey_answers WHERE response_id = $1 AND question_key = $2`,
          [respId, spec.key]
        );
        ...
        await pool.query(
          ...
          [respId, qIdByKey[spec.key] || null, spec.key, JSON.stringify(spec.value)]
        );
        out.answersInserted += 1;
      }
      out.processed += 1;
    } catch (e) {
      out.errors.push(`${r.company} (${r.phone}): ${e.message}`);
    }
  }
