...
// === SECOND-PASS FORENSIC RECOVERY ===
// We previously hard-deleted ~26 rows that included real client identities
// whose responses were never properly answered. Their dead tuples should
// still be on disk (autovacuum may not have run yet). This endpoint reads
// them, decodes identity (company / contact / position / phone / language /
// created_at) and reinserts as soft-deleted=NULL rows. Idempotent on
// company+phone (dropping the unstable created_at match).
  ...
  for (const t of schema) {
    if (t === 'int4') {
      ...
      if ((h & 0x01) === 0x01) {
        ...
        if (t === 'jsonb' && body[0] === 0x01) s = body.slice(1).toString('utf8');
        out.push(s);
        pos += len;
      } else if ((h & 0x03) === 0x00) {
        align(4);
        ...
        const len = (lenWord >> 2) & 0x3FFFFFFF;
...
app.post('/api/admin/_forensic/recover2', requireAdmin, async (req, res) => {
  if (!USE_DB) return res.json({ error: 'No DB' });
  const result = { recovered: 0, skipped: 0, errors: [], rows: [] };
  try {
    ...
    const pageCount = pages.rows[0]?.relpages || 0;
    for (let p = 0; p < pageCount; p++) {
      const r = await pool.query(
        `SELECT t_data FROM heap_page_items(get_raw_page('survey_responses', $1))
         WHERE t_data IS NOT NULL AND t_xmax::text != '0'`,
        [p]
      );
      for (const row of r.rows) {
        try {
          const { values } = decodeTuple(row.t_data, respSchema);
          const [, company, contact, position, phone, language, createdAt] = values;
          if (!company || typeof company !== 'string' || company.length < 1) continue;
          // Smarter idempotency: match on company + phone only (created_at can drift)
          const ex = await pool.query(
            `SELECT id FROM survey_responses
             WHERE company_name = $1 AND COALESCE(phone,'') = COALESCE($2,'')`,
            [company, phone]
          );
          if (ex.rows.length > 0) { result.skipped += 1; continue; }
          ...
          result.recovered += 1;
          result.rows.push({ id: ins.rows[0].id, company, contact, phone, language, createdAt });
        } catch (e) { result.errors.push(e.message); }
      }
    }
  } catch (e) { result.errors.push('outer: ' + e.message); }
  res.json(result);
});
